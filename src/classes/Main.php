...
class GBV extends hrwsBLZ
{
    ...
    public function suggestBanks($aBLZ = NULL){
        $res = BlzModel::where('blz', 'like', $this->blz_cleanblz($aBLZ).'%')->get();
        $this->specResponse = array(
            'blz' => $aBLZ,
			'bank' => $res,
			'status' => true,
    ...
    public function checkKtoBlz($kto, $blz){
		...
		elseif(isset($this->specResponse['msg'])){
			//
		}
		else{
			$this->specResponse['msg'] = 'KTO and BLZ Combination not valid!';
		}
        return $this->specResponse;
	}
	
    public function checkIBAN($iban){
		if($this->blz_checkKtoFromIBAN($iban)){
			$kto = substr($iban, 12, 10);
			$blz = substr($iban, 4, 8);
			return $this->checkKtoBlz($kto, $blz);
		}
		elseif(isset($this->specResponse['msg'])){
			//
		}
		else{
			$this->specResponse['msg'] = 'IBAN is not Valid!';
		}
        return $this->specResponse;
    }
